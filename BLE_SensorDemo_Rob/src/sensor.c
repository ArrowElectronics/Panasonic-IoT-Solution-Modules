...
#define DEFAULT_NAME_ALLMEMS		'B','C','N','-','R','O','B'
...
void Sensor_DeviceInit() {
	...
	uint8_t device_name[] = { 'B', 'C', 'N', '-', 'R', 'O', 'B' };
...
void Set_DeviceConnectable(void) {
	...
	uint8_t manuf_data[26] = { 2, /* lenght*/0x0A, 0x00 /* 0 dBm */,		// Transmission Power
			...
			0x02, /* Led + Prox */
			0xE2, /* Acc + Gyro + Mag + + Hum + Temp + Batt*/
#if ENABLE_MOTIONFX
...
void User_AppTick(void) {
  ...
  if (xFeatureNotification.BatteryMonitorNotification) {
